...
As mentioned, there are many similarities between two development ecosystems. We use Gradle to manage dependencies and build project just like we did with Composer. We use `routing` DSL instead `Route` to define application route. We use a `Map` of `TaskDto` to represent data structure just like we use `array` in Laravel. `Exposed` is pretty much the same with `Eloquent`. We can quickly get the notion by using metaphor above. All the source code could be found in this repository. I truly hope this article could help you get started quickly and enjoy your journey of Ktor development.
